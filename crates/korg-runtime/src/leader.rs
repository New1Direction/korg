...
pub use korg_registry::CognitionMode;
...
pub struct LeaderOrchestrator {
    ...
    pub tui_tx: Option<tokio::sync::mpsc::Sender<crate::tui_bridge::TuiUpdate>>,
    /// Channel to receive interactive feedback from the Ratatui TUI
    pub tui_rx: Option<tokio::sync::mpsc::Receiver<crate::tui_bridge::ContractResponse>>,
...
impl LeaderOrchestrator {
    ...
    pub fn handle_verdict(&mut self, verdict: &EvaluationVerdict) {
        ...
        if let Some(tx) = &self.tui_tx {
            ...
            let _ = tx.try_send(crate::tui_bridge::TuiUpdate::Verdict {
        ...
        if action == "revise" || action == "terminate_and_rollback" {
            if let Some(tx) = &self.tui_tx {
                let _ = tx.try_send(crate::tui_bridge::TuiUpdate::ApprovalRequest(
                    verdict
                        .justifications
                        .first()
                        .cloned()
        ...
        if verdict.doom_loop_detected {
            if let Some(tx) = &self.tui_tx {
                ...
                let failure_seq = journal.last_seq_id;
                if failure_seq > 0 {
    ...
    fn execute_rewind(&self, seq_id: u64) {
        ...
        match journal.rewind_with_seal(seq_id, "korg:operator", "operator-confirmed TUI rewind") {
            ...
            Err(e) => {
                tracing::error!("Rewind to seq {} failed: {}", seq_id, e);
                if let Some(tx) = &self.tui_tx {
                    let _ = tx.try_send(crate::tui_bridge::TuiUpdate::Trace(format!(
                        "[Korg] Rewind failed: {}",
                        e
    ...
    async fn persist_campaign_ktrans(
        ...
        let res = tokio::task::spawn_blocking(move || -> anyhow::Result<(String, crate::acp::MessageEnvelope<crate::acp::CampaignKtrans>)> {
            ...
            let state_merkle_root = if let Ok(content) = std::fs::read_to_string(korg_core::paths::blackboard_json()) {
            ...
            if let Some(ref content) = blackboard_content {
                let blob_dir = korg_core::paths::state_blobs_dir(&session_id).display().to_string();
            ...
            let dir = korg_core::paths::campaign_dir(&session_id).display().to_string();
            ...
            if let Some(tx) = &tui_tx {
                if let Ok(pretty) = serde_json::to_string(&envelope.payload) {
                    let _ = tx.try_send(crate::tui_bridge::TuiUpdate::Ktrans(pretty));
                }
            }
    ...
    pub async fn negotiate_contract(&mut self, plan: &serde_json::Value) -> Result<Contract> {
        ...
        if is_instant {
            ...
            let contract_dir = &korg_core::paths::contracts_dir().display().to_string();
            ...
            let bb_dir = &korg_core::paths::blackboard_dir().display().to_string();
        ...
        for round in 1..=max_rounds {
            ...
            if let (Some(tx), Some(rx)) = (&self.tui_tx, &mut self.tui_rx) {
                ...
                let _ = tx.try_send(crate::tui_bridge::TuiUpdate::ContractApprovalRequest {
                    round,
                    description: description.clone(),
                ...
                if let Some(response) = rx.recv().await {
                    match response {
                        ...
                        crate::tui_bridge::ContractResponse::Rewind(seq_id) => {
                            self.execute_rewind(seq_id);
                        }
                    }
                }
            }
        ...
        if let Some(tx) = &self.tui_tx {
            ...
            let _ = tx.try_send(crate::tui_bridge::TuiUpdate::ContractNegotiated {
    ...
    fn load_blackboard() -> (serde_json::Value, String) {
        let bb_path = korg_core::paths::blackboard_json();
    ...
    async fn run_observable_campaign_internal(&mut self) -> Result<()> {
        ...
        korg_llm::CAMPAIGN_TOKENS.store(0, std::sync::atomic::Ordering::Relaxed);
        ...
        loop {
            ...
            for round in 0..10 {
                ...
                if let Some(ref mut r) = rx {
                    while let Ok(response) = r.try_recv() {
                        match response {
                            crate::tui_bridge::ContractResponse::Override(override_vec) => {
                            ...
                            crate::tui_bridge::ContractResponse::Rewind(seq_id) => {
                                self.execute_rewind(seq_id);
                                restart_with_fork = Some((0, format!("REWIND:{}", seq_id)));
                            }
                            _ => {}
                        }
                    }
                ...
                if let Some(tx) = &self.tui_tx {
                    ...
                    let total_tokens =
                        korg_llm::CAMPAIGN_TOKENS.load(std::sync::atomic::Ordering::Relaxed);
                    let comps =
                        korg_llm::COMPLETIONS_COUNT.load(std::sync::atomic::Ordering::Relaxed);
                    let tot_lat =
                        korg_llm::TOTAL_LATENCY_MS.load(std::sync::atomic::Ordering::Relaxed);
                ...
                if let Some(tx) = &self.tui_tx {
                    let _ = tx.try_send(crate::tui_bridge::TuiUpdate::Ktrans(format!(
                        "round {} | {} | swarm={}",
                ...
                if round % 5 == 0 {
                    if let Some(tx) = &self.tui_tx {
                        let _ = tx.try_send(crate::tui_bridge::TuiUpdate::Compaction(format!(
            ...
            let campaign_path = korg_core::paths::campaign_dir(&self.session_id)
                .display()
                .to_string();
    ...
    pub async fn handle_operator_fork(&mut self, tx_id: usize, directive: &str) -> Result<()> {
        let dir = korg_core::paths::campaign_dir(&self.session_id)
            .display()
            .to_string();
        ...
        if let Some(target) = target_ktrans {
            ...
            let state_blob_path = korg_core::paths::state_blobs_dir(&self.session_id)
        ...
        let forks_dir = korg_core::paths::forks_dir(tx_id).display().to_string();
        ...
        // Log the split
        let fork_log = format!("[Fork] Split occurred at playhead tx_{:02}. Directive: '{}'. Reverted Blackboard snapshot written to {}. Sandbox path: {}", tx_id, directive, korg_core::paths::blackboard_json().display(), forks_dir);
        println!("{}", fork_log);
        if let Some(tx) = &self.tui_tx {
            let _ = tx.try_send(crate::tui_bridge::TuiUpdate::Trace(fork_log));
        }
    ...
    pub fn replay_campaign(&self, session: Option<Uuid>) -> Result<()> {
        ...
        let dir = korg_core::paths::campaign_dir(&sid).display().to_string();
...
"#,
        ...
        let cfg = korg_llm::KorgConfig::load();
        let provider = korg_llm::build_provider(&cfg);

        let messages = vec![
            korg_llm::Message {
                role: korg_llm::Role::System,
        ...
        let campaign_dir = korg_core::paths::campaign_dir(&self.session_id)
            .display()
            .to_string();
    ...
    async fn merge_received_ktrans(&self, routing_ids: &[String]) {
        let ktrans_dir = &korg_core::paths::ktrans_dir().display().to_string();
        let bb_dir = &korg_core::paths::blackboard_dir().display().to_string();
        let _ = tokio::fs::create_dir_all(bb_dir).await;
    ...
    async fn prompt_operator_escalation(&mut self, error_msg: &str) -> Result<OperatorChoice> {
        ...
        if let (Some(tx), Some(rx)) = (&self.tui_tx, &mut self.tui_rx) {
            let _ = tx.try_send(crate::tui_bridge::TuiUpdate::ApprovalRequest(msg.clone()));
            println!("⏳ [Leader] Waiting for operator decision in TUI/Web...");
            if let Some(response) = rx.recv().await {
                match response {
                    crate::tui_bridge::ContractResponse::Approve => {
...
mod tests {
    ...
    async fn test_arena_and_semantic_merge() {
        ...
        let merge_path = korg_core::paths::semantic_merge_path(&leader.session_id)
            .display()
            .to_string();
    ...
    async fn test_merkle_dag_ledger_integrity() {
        ...
        let dir = korg_core::paths::campaign_dir(&session_id)
            .display()
            .to_string();
    ...
    async fn test_playhead_steering_fork_campaign_reset() {
        ...
        feedback_tx
            .try_send(crate::tui_bridge::ContractResponse::Override(vec![
