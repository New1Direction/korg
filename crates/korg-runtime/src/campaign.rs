...
pub async fn persist_campaign_ktrans(
    ...
    korg_core::metrics::record_ktrans_persisted(round);
...
pub async fn persist_final_summary_ktrans(
    ...
    korg_core::metrics::record_campaign_completed();
    Ok(tx_hash)
}
...
async fn capture_state_merkle_root(session_id: Uuid) -> String {
    let content = tokio::fs::read_to_string(korg_core::paths::blackboard_json()).await;
...
async fn persist_state_blob(session_id: Uuid, state_merkle_root: &str) {
    let blob_dir = korg_core::paths::state_blobs_dir(&session_id);
    let _ = tokio::fs::create_dir_all(&blob_dir).await;
    let blob_path = blob_dir.join(format!("{}.json", state_merkle_root));
    if let Ok(content) = tokio::fs::read_to_string(korg_core::paths::blackboard_json()).await {
