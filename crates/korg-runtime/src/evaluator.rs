...
use korg_embeddings::{cosine_similarity, EmbeddingModel};
...
impl Evaluator {
    pub fn new(config: Option<RubricConfig>) -> Self {
        ...
        let embedding_model: std::sync::Arc<dyn korg_embeddings::EmbeddingModel> =
            match korg_embeddings::CandleEmbeddingModel::load() {
                ...
                Err(e) => {
                    ...
                    std::sync::Arc::new(korg_embeddings::FakeEmbeddingModel::default())
                }
            };
    ...
    pub async fn evaluate(&mut self, session_id: Uuid) -> EvaluationVerdict {
        ...
        korg_core::metrics::record_evaluator_verdict(&overall, doom, live_entropy);
