...
use korg_llm::{LlmProvider, LlmRequest, Message, Role};
use serde_json::json;
use std::fs;
use std::sync::Arc;
use uuid::Uuid;
...
pub fn load_prompt_for_persona(persona: Persona) -> String {
    ...
    let path = korg_core::paths::prompts_dir().join(filename);
...
impl LlmPersona {
    ...
    pub async fn think(
        ...
        payload: &str,
        routing_id: &str,
    ) -> Result<PersonaResult, korg_llm::LlmError> {
        let messages = vec![
            Message {
                role: Role::System,
...
pub async fn run_persona(persona: Persona, payload: &str, routing_id: &str) -> PersonaResult {
    ...
    let (provider, temp_override) = korg_llm::build_provider_for_persona(&cfg, persona.name());
...
mod tests {
    use super::*;
    use korg_llm::{LlmResponse, MockProvider, Role};
    use serde_json::json;
