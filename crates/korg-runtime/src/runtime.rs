...
pub struct CampaignRuntime {
    ...
    pub initial_capabilities: HashMap<String, korg_registry::CapabilityState>,
}
...
impl CampaignRuntime {
    ...
    pub async fn snapshot_capabilities(&mut self, resolver: &korg_registry::CapabilityResolver) {
    ...
    pub async fn rollback_capabilities(
        &self,
        resolver: &Arc<tokio::sync::Mutex<korg_registry::CapabilityResolver>>,
    ) {
    ...
    async fn test_campaign_runtime_execute_abort_sequence() {
        ...
        ));
        {
            ...
            res.active_states.insert(
                ...
                korg_registry::CapabilityState::Mode("balanced".to_string()),
            );
        }
        ...
        assert_eq!(
            ...
            Some(&korg_registry::CapabilityState::Mode(
                "balanced".to_string()
            ))
        );
        ...
        // 3. Mutate the active states in the resolver
        {
            ...
            res.active_states.insert(
                "cognition_mode".to_string(),
                korg_registry::CapabilityState::Mode("heavy".to_string()),
            );
        }
