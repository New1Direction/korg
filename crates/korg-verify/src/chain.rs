//! korg-ledger@v1 — tamper-evident hash-chain (vendored standalone copy).
//!
//! This is `korg-verify`'s own copy of the frozen `korg-ledger@v1` chain
//! primitives, vendored here so the verifier builds and publishes with **zero
//! workspace dependencies** (`cargo install korg-verify` pulls in nothing from
//! this repo). It is byte-identical to the `korg-registry` core at vendor time;
//! the two are kept honest by the SHARED frozen conformance vectors in
//! `tests/conformance/` — if either copy ever drifts, the pinned tip hashes in
//! `tests/conformance.rs` stop reproducing and the build goes red. That shared
//! oracle is exactly what makes this an *independent* implementation rather than
//! a thin alias of the core.
//!
//! Canonical spec text: `spec/korg-ledger-v1/SPEC.md`. Sibling implementations
//! (Python reference `spec/korg-ledger-v1/conformance.py`, JS verifier
//! `spec/korg-ledger-v1/js/verify.mjs`) reproduce the same vectors.
//!
