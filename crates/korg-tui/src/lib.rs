                    ...
                    if app.rewind_mode {
                        match key.code {
                            KeyCode::Up | KeyCode::Char('k') => {
                                if app.rewind_cursor > 0 {
                                    app.rewind_cursor -= 1;
                                }
                            }
                            KeyCode::Down | KeyCode::Char('j') => {
                                if app.rewind_cursor + 1 < app.rewind_candidates.len() {
                                    app.rewind_cursor += 1;
                                }
                            }
                            KeyCode::Enter => {
                                    ...
                                    app.rewind_candidates.get(app.rewind_cursor)
                                {
                                    ...
                                    app.rewind_mode = false;
                                    app.rewind_candidates.clear();
                                }
                            }
                            KeyCode::Esc => {
                                app.rewind_mode = false;
                                app.rewind_candidates.clear();
                                app.log("Rewind dismissed — campaign continues");
                            }
                            _ => {}
                        }
                        continue;
                                                ...
                                                if cmd_line == "/help" {
                                                        ...
                                                        (path_opt, content_opt)
                                                    {
                                                        ...
                                                        tokio::spawn(async move {
                                                            ...
                                                            let result = korg_runtime::personas::run_persona(
                                                                korg_runtime::personas::Persona::Captain,
                                                                &prompt,
                                                                "tui-explain"
                                                            ).await;
                                                        ...
                                                        (path_opt, content_opt)
                                                    {
                                                        ...
                                                        tokio::spawn(async move {
                                                            ...
                                                            let result = korg_runtime::personas::run_persona(
                                                                korg_runtime::personas::Persona::Harper,
                                                                &prompt,
                                                                "tui-critique"
                                                            ).await;
                                                            let reply = if let Some(explanation) =
                ...
                0 => vec![
                    ...
                    Line::from(Span::styled(
                        format!(
                            ...
                            korg_core::paths::project_root().display()
                        ),
                        Style::default().fg(fg_slate),
                    )),
                ],
